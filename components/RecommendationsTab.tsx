...
import { StarterStack, IntegratedInsight, AllPractice, IntelligentGuidance, PersonalizationSummary } from '../types.ts';
        ...
        {intelligentGuidance && (
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              {/* Practice Changes - Enhanced with Sequencing */}
              {intelligentGuidance.recommendations.practiceChanges?.add && intelligentGuidance.recommendations.practiceChanges.add.length > 0 && (
                <div>
                  <h4 className="text-md font-semibold text-slate-200 mb-3">Recommended Practices to Add</h4>
                  <div className="space-y-3">
                    {intelligentGuidance.recommendations.practiceChanges.add.map((rec, idx) => (
                      <button
                        key={idx}
                        onClick={() => rec.practice && addToStack(rec.practice)}
                        className="w-full text-left bg-slate-700/50 hover:bg-slate-700 border border-slate-600/50 hover:border-purple-500/50 rounded-lg p-4 transition group"
                      >
                        {/* Header with sequence and priority */}
                        <div className="flex items-start justify-between mb-3">
                            ...
                            {rec.sequenceGuidance && (
                              <div className="flex items-start gap-2 text-xs">
                                ...
                                <div>
                                  ...
                                  <p className="text-slate-200">{rec.sequenceGuidance}</p>
                                </div>
                              </div>
                            )}
                            {rec.timeCommitment && (
                              <div className="flex items-start gap-2 text-xs">
                                ...
                                <div>
                                  ...
                                  <p className="text-slate-200">{rec.timeCommitment}</p>
                                </div>
                              </div>
                            )}
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <ArrowRight size={16} className="text-slate-400 group-hover:text-purple-400 transition"/>
                        </div>
                      </button>
                    ))}
                  </div>
